...
import colorama
import telegram
from telegram.constants import MAX_TEXT_LENGTH
    ...
    if messages:
        ...
        for m in messages:
            if len(m) > MAX_TEXT_LENGTH:
                warn(markup(
                    f"Message longer than MAX_TEXT_LENGTH={MAX_TEXT_LENGTH}, splitting into smaller messages.",
                    "red"))
                ms = split_message(m, MAX_TEXT_LENGTH)
                for m in ms:
